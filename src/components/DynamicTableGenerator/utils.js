...
let token = "";
let domain_name = "";
export function change(params, params2, params3) {
  specs = params;
  token = params2;
  domain_name = params3;
  console.log(specs);
          ...
          axios
              ...
              .then(res => {
                ...
                for (let i = 0; i < keys.length; i++) {
                  ...
                  virtualDom$option_specs.className =  `select${ keys[i] }`;
      ...
      for (let j = 0; j < th_content.length; j++) {
        ...
        switch (j) {
          ...
          case 2:
            ...
            let dom$input_goodsPics = virtualDom$td_common.appendChild(virtualDom$input_goodsPics);
            virtualDom$input_goodsPics.addEventListener("change", function (ev) {
              ...
              axios({
                ...
                method: 'POST',
                  ...
                  .then((res) => {
                    // virtualDom$input_goodsPics.value = `${ domain_name }${ res.data.key }`;
                    // virtualDom$input_goodsPics.innerHTML = `${ domain_name }${ res.data.key }`;
                    console.log(`url: ${ domain_name }${ res.data.key }`);
                    
                    dom$input_goodsPics.val = `${ domain_name }${ res.data.key }`;
                  })
                  .catch((err) => {
                    console.warn(err.message);
                  });
          ...
          for (let j = 0; j < trs[i].length; j++) {
            ...
            if (dom$td_thirdSpecs !== undefined) {
              ...
              let data_object = {
                ...
                goods_pics: goods_pics !== undefined ? goods_pics.val : '无',
              };
...
export function getGoodsSpecsData() {
