...
export default {
  ...
  effects: {
    ...
    *tableUploadPic({ payload: param }, { call, put }) {
      // let token = "2qKDRQz2JQb45aZjHAmER79xZ3stLOOvSHEq8n34:IjmSdPp63vP-A0mJKeIL-ZWZMmc=:eyJzY29wZSI6Imh1aXl1bi1hcHAtaW1hZ2VzIiwiZGVhZGxpbmUiOjE1Nzg4OTQ3MTR9";
  ...
  reducers: {
      ...
      for(let i = 0; i < param.specs.length; i++) {
        ...
        if (len < config_common_properties.COUNT_CONTAINER && len >= 0) {
          ...
          for(let j = 0; j < param.specs[i].spec_option.length; j++) {
            ...
            console.log("awsd");
            console.log(param.specs[i].spec_option);
  
            console.log(param.specs[i].spec_option[j].id, param.specs[i].spec_option[j].name, param.specs[i].spec_option[j].id);
            
            
            a.push({
              // prop: specs[i].spec_option[j].id,
              // name: specs[i].spec_option[j].name,
              // id: specs[i].spec_option[j].id,
    ...
    _removeReal_childNames(state, { payload: param }) {
      ...
      setTimeout(() => {
        ...
        console.log(Number(param.item.child_id));
        console.log(common);
